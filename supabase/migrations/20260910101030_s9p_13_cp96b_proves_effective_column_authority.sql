-- S9-P/13: CP-96b must prove EFFECTIVE column authority, not direct grants.
--
-- WHAT WAS WRONG. CP-96b asserted that no row existed in
-- information_schema.column_privileges for grantee 'authenticated' with
-- privilege_type INSERT or UPDATE, and carried the claim that column-level
-- authority was absent. Those are not the same statement. That view lists
-- DIRECT grants: a grant made to a role that `authenticated` INHERITS does not
-- appear under the grantee 'authenticated' at all, so the gate would stay green
-- while effective column authority existed through membership in another role.
-- The gate proved something narrower than the sentence it printed, which is the
-- worst failure mode available to a test - it reads like a control and is not
-- one.
--
-- WHAT REPLACES IT. has_column_privilege() answers the effective question: it
-- resolves role inheritance, and it also returns true when a table-level grant
-- implies the column. Every live column of public.batches is enumerated from
-- pg_attribute and both INSERT and UPDATE are checked on each.
--
-- CP-96b0 IS NOT PADDING. A `count(*) = 0` assertion over an enumeration passes
-- just as happily when the enumeration is empty. CP-96b0 proves the column list
-- is non-empty first, so the zero in CP-96b can only mean absence of privilege.
--
-- THE DIRECT-GRANT CHECK IS KEPT, DEMOTED. CP-96d retains the catalogue query as
-- corroboration - it names the narrower fact accurately now, and it would still
-- catch a direct grant that somehow evaded the effective check. It no longer
-- stands in for the effective claim.
--
-- NOTHING ELSE IN THE SUITE CHANGES. This is a full CREATE OR REPLACE because
-- PL/pgSQL has no partial redefinition; the amended region is the CP-96b block
-- and nothing else. That is verified after the fact by reversing the
-- substitution and comparing against the previous body hash, rather than
-- asserted.

create or replace function tests.__s9p_body()
...
begin
  ...
  -- CP-31a REWRITTEN (D-U). The authenticated path no longer reaches the foreign
  -- key at all: S9-P/9 removed every direct write privilege on batches, so the
  -- refusal is a privilege refusal and that is what is asserted here.
  perform pg_catalog.set_config('request.jwt.claims', v_mclaims, true);
  set local role authenticated;
  begin
  ...
  return next is(v_state, '42501',
    'CP-31a a DIRECT authenticated write of the Release is refused for want of PRIVILEGE - after S9-P/9 the authenticated path never reaches the constraint');

  -- CP-31b (D-U). The composite foreign key is still proved, independently, at
  -- OWNER level where privileges do not apply - inside a subtransaction always
  -- rolled back by a sentinel, so an unexpected success cannot survive.
  begin
    update public.batches set pricing_basis_release_id = v_rel_nag where id = v_batch;
    raise exception 'S9P sentinel' using errcode = '55000';
  exception
    when sqlstate '55000' then v_con := 'NO CONSTRAINT FIRED';
    when others then get stacked diagnostics v_con = constraint_name;
  end;
  return next is(v_con, 'fk_batch_pricing_basis',
    'CP-31b and the composite foreign key itself still refuses a foreign-plant Release BY NAME, proved privileged and rolled back - integrity does not depend on the privilege');
  ...
  -- ═══════════════ E. direct write authority on batches is gone (S9-P/9) ══
  select to_jsonb(b) into v_before from public.batches b where id = v_batch;
  perform pg_catalog.set_config('request.jwt.claims', v_mclaims, true);
  set local role authenticated;
  begin
    update public.batches set pricing_basis_release_id = v_rel_alt where id = v_batch;
    v_state := 'NO ERROR';
  exception when others then v_state := sqlstate;
  end;
  reset role;
  ...
  return next is(v_state, '42501',
    'CP-90 a lock-holding Maker cannot set pricing_basis_release_id directly - 42501');
  return next ok(v_before = v_after,
    'CP-90a and the COMPLETE Batch row is identical afterwards');

  select to_jsonb(b) into v_before from public.batches b where id = v_batch;
  perform pg_catalog.set_config('request.jwt.claims', v_mclaims, true);
  set local role authenticated;
  begin
    update public.batches set pricing_basis_is_deliberate = true where id = v_batch;
    v_state := 'NO ERROR';
  exception when others then v_state := sqlstate;
  end;
  reset role;
  select to_jsonb(b) into v_after from public.batches b where id = v_batch;
  return next is(v_state, '42501',
    'CP-91 nor flip pricing_basis_is_deliberate on its own - the provenance flag can no longer assert a choice nobody made');
  return next ok(v_before = v_after, 'CP-91a and the COMPLETE row is unchanged');

  select to_jsonb(b) into v_before from public.batches b where id = v_batch;
  perform pg_catalog.set_config('request.jwt.claims', v_mclaims, true);
  set local role authenticated;
  begin
    update public.batches set pricing_date = date '1999-01-01' where id = v_batch;
    v_state := 'NO ERROR';
  exception when others then v_state := sqlstate;
  end;
  reset role;
  select to_jsonb(b) into v_after from public.batches b where id = v_batch;
  return next is(v_state, '42501', 'CP-92 nor write an arbitrary Pricing Date');
  return next ok(v_before = v_after, 'CP-92a and the COMPLETE row is unchanged');

  select to_jsonb(b) into v_before from public.batches b where id = v_batch;
  perform pg_catalog.set_config('request.jwt.claims', v_mclaims, true);
  set local role authenticated;
  begin
    update public.batches set status = 'sent', pricing_date = date '1999-01-01' where id = v_batch;
    v_state := 'NO ERROR';
  exception when others then v_state := sqlstate;
  end;
  reset role;
  select to_jsonb(b) into v_after from public.batches b where id = v_batch;
  return next is(v_state, '42501',
    'CP-93 a MIXED statement naming a pre-existing S6 column and a governed pricing column fails as a unit - after full revocation neither half is writable');
  return next ok(v_before = v_after,
    'CP-93a and NEITHER column moved - the failure is atomic, not partial');

  select to_jsonb(b) into v_before from public.batches b where id = v_batch;
  perform pg_catalog.set_config('request.jwt.claims', v_mclaims, true);
  set local role authenticated;
  begin
    update public.batches set status = 'sent' where id = v_batch;
    v_state := 'NO ERROR';
  exception when others then v_state := sqlstate;
  end;
  reset role;
  select to_jsonb(b) into v_after from public.batches b where id = v_batch;
  return next is(v_state, '42501',
    'CP-94 a direct working -> sent status change is refused - the Atomic Send belongs to S9(b) and can no longer be bypassed by a PATCH');
  return next ok(v_before = v_after, 'CP-94a and the Batch is still working, unchanged');

  perform pg_catalog.set_config('request.jwt.claims', v_mclaims, true);
  set local role authenticated;
  begin
    insert into public.batches (batch_reference, family_id, plant_id, owner_user_id,
                                status, created_by, pricing_date)
    values ('pending', v_fam, v_kol, v_maker, 'working', v_maker, v_today);
    v_state := 'NO ERROR'; v_con := null;
  exception when others then v_state := sqlstate; v_con := sqlerrm;
  end;
  reset role;
  return next is(v_state, '42501', 'CP-95 a direct INSERT into batches is refused');
  return next ok(v_con like '%permission denied%',
    'CP-95a for want of PRIVILEGE, not by policy - the refusal is now the intended one');

  return next ok(
    pg_catalog.has_table_privilege('authenticated','public.batches','SELECT'),
    'CP-96 authenticated retains SELECT on batches - visibility is unchanged and still row-scoped by batches_select');
  return next ok(
    not (pg_catalog.has_table_privilege('authenticated','public.batches','INSERT')
      or pg_catalog.has_table_privilege('authenticated','public.batches','UPDATE')),
    'CP-96a and holds NO effective table-level INSERT or UPDATE');

  -- CP-96b AMENDED (S9-P/13). information_schema.column_privileges lists DIRECT
  -- grants only: authority reaching `authenticated` through membership in some
  -- other role never appears under that grantee, so the old form could stay
  -- green while effective column authority existed. has_column_privilege gives
  -- the effective answer - it resolves inheritance, and returns true when a
  -- table-level grant implies the column. CP-96b0 first proves the enumeration
  -- is non-empty, because a count of zero over an empty set proves nothing.
  select count(*)::int into v_n
    from pg_catalog.pg_attribute a
   where a.attrelid = 'public.batches'::regclass
     and a.attnum > 0 and not a.attisdropped;
  return next ok(v_n > 0,
    format('CP-96b0 the column enumeration is non-empty (%s live columns) - so the zero below means absence of PRIVILEGE, not absence of columns', v_n));
  return next is(
    ...
    0,
    'CP-96b NO live batches column confers EFFECTIVE INSERT or UPDATE on authenticated - has_column_privilege resolves authority inherited through any role, which a catalogue of direct grants cannot see');
  return next is(
    (select count(*)::int from information_schema.column_privileges
      where table_schema='public' and table_name='batches'
        and grantee='authenticated' and privilege_type in ('INSERT','UPDATE')),
    0,
    'CP-96d and no DIRECT column grant exists either - the catalogue corroborates CP-96b and would catch a grant made straight to authenticated, but it does not substitute for the effective check');

  return next is(
    (select count(*)::int from pg_catalog.pg_policy pol
       join pg_catalog.pg_class c on c.oid = pol.polrelid
       join pg_catalog.pg_namespace n on n.oid = c.relnamespace
      where n.nspname='public' and c.relname='batches' and pol.polcmd <> 'r'),
    0,
    'CP-96c and batches carries no INSERT or UPDATE policy - a policy with no privilege behind it would be dead code that reads like a control');
  return next is(
    (select count(*)::int from pg_catalog.pg_attribute
      where attrelid='public.batches'::regclass and attname='pricing_date' and atthasdef),
    0,
    'CP-97 batches.pricing_date has NO default - every insert path must supply the plant-local date explicitly (D-V)');

  -- ═══════════════════════════ boundary: what this tranche did NOT build ══
  return next is(
