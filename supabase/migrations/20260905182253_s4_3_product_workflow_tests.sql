-- S4-3: executable proof gates for the product-definition workflow.
--
-- These exercise the operations end to end as two real personas - a Maker who
-- may propose and nothing more, and an NPD/Librarian who may approve, publish and
-- adopt. Every denial is asserted from the persona's own session, not inferred
...
declare
  ...
  f text; v_ok boolean; v_err text;
  ...
  v_sku bigint; v_sver bigint; v_n int;
begin
  ...
  update public.constructions set surviving_construction_id = null where name like '\_\_p2 pw%';
...
revoke all on function tests.product_workflow() from public;
revoke all on function tests.product_workflow() from anon;
revoke all on function tests.product_workflow() from authenticated;