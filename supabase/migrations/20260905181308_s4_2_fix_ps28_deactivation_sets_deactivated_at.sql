-- S4-2 fix: PS-28 set status='deactivated' without deactivated_at, which
-- ck_app_users_deactivated refuses - the constraint makes the act and its
-- timestamp inseparable, exactly as ck_cv_approval_pair does for approval.
-- The assertion was right; the fixture was writing an impossible row.
--
-- Only the deactivation round-trip changes. Every assertion is untouched.
...
begin
  ...
  -- ck_app_users_deactivated makes the act and its timestamp inseparable, so both
  -- move together in each direction.
