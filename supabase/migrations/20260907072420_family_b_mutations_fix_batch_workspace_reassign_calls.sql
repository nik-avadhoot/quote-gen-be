-- Fix: tests.batch_workspace() (BF-13) called the old 3-arg
-- reassign_party_family(party, family, date), which no longer exists after
-- the Family B mutations slice added mandatory CAS. Reads the Party's
-- content_version before each of the two calls (there and back) and passes
-- it as the new required argument. Nothing else in this function changes.
-- (Recorded here verbatim from the already-verified live definition, having
-- been applied moments earlier via a self-verifying regexp-patch DO block
-- that aborts instead of silently no-op-ing on a non-match - that approach
-- is not idempotent to reapply, so this migration records its result.)
