-- S9-P/8: register the persistence suite.
--
-- tests.suite_registration() enforces that every tests.* suite is reachable from
-- run_all(), so an unregistered suite is itself a failure. Registration is
-- therefore not bookkeeping - it is the thing that stops a suite from silently
-- never running.
--
-- The new suite is placed AFTER tests.quote_schema(), which is the regression
-- net for the Family G immutability guarantees. Order matters only in that
-- calculation_persistence mints Releases at KOL, and tests.pricing_basis() mints
-- an automatic default at NAG; the two cannot collide because they use different
-- plants, and the suite documents why.

create or replace function tests.run_all()
returns setof text language plpgsql set search_path = 'extensions', 'pg_catalog' as $function$
...
begin
  ...
  return query select * from tests.calculation_persistence();
