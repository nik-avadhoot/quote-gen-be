-- Fix: the suite called app_private.merge_customer_families /
-- app_private.reassign_customer_family, which do not exist - those names
-- are the PUBLIC wrappers only. The app_private functions are still named
-- merge_families / reassign_party_family (unchanged names, changed
-- signatures). Direct DB-layer testing calls app_private.* directly.
...
begin
  ...
  set local role authenticated;
  begin
    perform app_private.reassign_party_family(v_party, v_fam3, v_cv - 1, current_date);
    reset role;
