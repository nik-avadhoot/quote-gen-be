-- Fix: v_admin's grants must include read_party_master too, since one
-- in-block SELECT (between set local role authenticated / reset role, in the
-- alias update-then-retire sequence) runs subject to RLS as 'authenticated'
-- and needs to read customer_family_aliases back.
