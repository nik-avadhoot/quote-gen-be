-- ck_app_users_deactivated enforces (status = 'deactivated') = (deactivated_at is not null).
-- The UC-17/UC-18 fixture flipped status directly without setting deactivated_at,
-- so it violated the constraint before the assertion it was setting up could run.
-- Corrected in place; the assertions themselves are unchanged.
...
begin
  ...
  update public.app_users
     set status = 'deactivated', deactivated_at = now() where id = v_admin2;
  ...
  update public.app_users
     set status = 'active', deactivated_at = null where id = v_admin2;
