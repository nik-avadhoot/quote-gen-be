-- S4-1 fix: the Construction Library fixture removed its app_user before the
-- invitation that references it, so cleanup tripped fk_invite_consumed_by.
--
-- The accepted order is the one P2-9 already uses (20260905075810:112-113):
-- the invitation goes first, because consumed_by points AT the app_user. Only
-- the cleanup block changes; every assertion is untouched.
