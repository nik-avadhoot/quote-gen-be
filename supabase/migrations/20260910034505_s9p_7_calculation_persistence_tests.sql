-- S9-P/7: the persistence proof gates.
--
-- WHAT A GREEN RUN HERE DOES AND DOES NOT MEAN. Every gate below proves that a
-- value can be STORED, DISTINGUISHED and GOVERNED. Not one of them proves that a
-- freshness test works, because no fingerprint builder exists yet - that is
-- S7-R's, and the hash-dependent obligations (CP-2, CP-10..CP-25, CP-50..CP-54)
-- are deliberately absent from this file. So are CP-67 and CP-72, which belong
-- to Send. A reader must not mistake this suite's green for proof of freshness.
--
-- WHY KOL AND NOT NAG. tests.pricing_basis() approves an automatic default
-- Release at NAG effective 2026-01-01 with no end date, so it covers today.
-- ex_pbr_default_no_overlap forbids a second approved automatic default
-- overlapping it at the same plant, so minting one at NAG would fail against a
-- sibling suite's fixture rather than against anything this tranche did. KOL is
-- used by the multi-plant and plant-master suites but by no Release fixture, so
-- it is free. The suite still MINTS every identity it uses; it borrows none.
--
-- THE GAP BATCH IS CREATED FIRST, ON PURPOSE. CP-37 needs a plant with no
-- automatic default covering today, and the moment the Release fixtures exist
-- KOL has one. Creating that Batch before them is the only ordering in which the
-- calendar-gap path is reachable at all.
--
-- EVERY WRITE THAT RLS GOVERNS RUNS AS A PERSONA. batches, batch_rows and
-- pricing_groups are all FORCE RLS, so an owner-level UPDATE against them is
-- filtered to zero rows rather than refused - which would turn a constraint
-- probe into a silent no-op that passes for the wrong reason. The direct-write
-- probes below therefore run as the lock-holding Maker.
...
begin
  ...
  -- ─────────────────────────────────────── approved commercial components
  ...
  -- NOT naming fluting_bcf_default: S9-P/2 keeps a column default precisely so
  -- an existing fixture insert like this one keeps working (the S7-1 precedent).
  ...
  -- ══════════════════════════════════ CP-38 the exclusion actually holds ══
  perform pg_catalog.set_config('request.jwt.claims', v_pclaims, true);
  set local role authenticated;
  ...
  -- ══════════════════════════ CP-30 automatic selection at creation ══
  perform pg_catalog.set_config('request.jwt.claims', v_mclaims, true);
  set local role authenticated;
  ...
  -- ═══════════════════════════════ A. storage distinguishes blank from zero ══
  perform pg_catalog.set_config('request.jwt.claims', v_mclaims, true);
  set local role authenticated;
  ...
  -- the direct write runs as the lock-holding Maker: batches is FORCE RLS, so an
  -- owner-level update would be filtered to zero rows and pass for the wrong reason
  perform pg_catalog.set_config('request.jwt.claims', v_mclaims, true);
  set local role authenticated;
  begin
  ...
  -- the boundary: effective_until EQUAL to the Pricing Date is COVERED, because
  -- daterange(from, until, '[]') is inclusive at both ends. An off-by-one here
  -- would price a Quote on the wrong basis with nothing visible to anyone.
  perform pg_catalog.set_config('request.jwt.claims', v_mclaims, true);
  set local role authenticated;
