-- S6-8: register the three Family F suites and drop the development probes, so
-- tests.run_all() is once again the single command that states the position.
...
drop function if exists tests.__probe_bf();
drop function if exists tests.__probe_s6();