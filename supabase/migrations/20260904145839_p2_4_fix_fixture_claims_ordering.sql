-- P2-4 defect correction 2: the fixture allocated a Group Customer Code before any
-- JWT claims were set, so ref_private.allocate_reference correctly refused with
-- "no active app user". The guard is right; the fixture called it too early.
-- Claims are now established immediately after the maker fixture becomes active.
--
-- Only the ordering changed. Every assertion is identical.
...
begin
  ...
  -- claims must exist before any ref_private allocation: current_app_user() reads them
  perform pg_catalog.set_config('request.jwt.claims', v_claims, true);
