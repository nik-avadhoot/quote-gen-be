-- S7-R/9: CP-100 … CP-116, plus the D-X / D-W / D-G rulings.
--
-- THE GATE THIS SUITE EXISTS FOR IS CP-100. The withdrawn Revision 1 accepted
-- p_results from an authenticated caller and defended it with three internal
-- identities. Those constrain arithmetic, not magnitude: a consistently scaled
-- set satisfies every one of them, and because the fingerprint is computed from
-- durable state a tampered payload would be stamped FRESH and pass Send's
-- staleness test cleanly. CP-100 and CP-101d are written to fail that design.
--
-- THE TEST KEY EXISTS ONLY INSIDE THE TRANSACTION THAT RUNS THIS SUITE, and the
-- teardown removes it. CP-115 asserts the persistent keyring is empty AFTER the
-- teardown - it lives in tests.calculation_writer(), because an assertion about
-- what is left behind cannot be made before the leaving.
--
-- tests.__s7r_sign IS THE TRUSTED EXECUTOR'S STAND-IN. It signs the same
-- fourteen framed fields the verifier rebuilds. What it does NOT stand in for
-- is the Edge runtime: nothing here executes the JavaScript engine, so the
-- results payloads below are fixtures chosen to satisfy the contract, not
-- engine output. NO END-TO-END CLAIM IS MADE BY THIS SUITE.
--
-- D-X NEEDS A REAL CHECKER. can_write_batch refuses a Maker on a SUBMITTED
-- Batch before the ruling is ever reached, so testing D-X with a Maker would
-- pass for the wrong reason. The suite mints a check_quote holder, proves
-- can_write_batch ADMITS them, and only then proves Calculate refuses.

create or replace function tests.__s7r_sign(
  p_keyid text, p_key bytea, p_sub text, p_user bigint, p_batch bigint,
  p_row bigint, p_cv integer, p_rel bigint, p_eng text, p_cfp text, p_pfp text,
  p_text text, p_ca timestamptz, p_ea timestamptz)
returns text language sql stable set search_path = 'extensions', 'pg_catalog' as $$
  select 'qca/1~' || p_keyid || '~' || app_private.qca_ts(p_ca) || '~'
      || app_private.qca_ts(p_ea) || '~'
      || encode(extensions.hmac(
           app_private.qca_mac_input(p_keyid, p_sub, p_user, p_batch, p_row, p_cv,
             p_rel, p_eng, p_cfp, p_pfp,
             encode(sha256(convert_to(p_text,'UTF8')),'hex'), p_ca, p_ea),
           p_key, 'sha256'), 'hex')
$$;
...
begin
  ...
  update public.skus set status='discontinued', plant_item_code='PIC-S7R-2', replacement_sku_id=v_sku where id=v_sku2;
  insert into public.sku_versions (sku_id,plant_id,version_no,construction_version_id,is_price_driving,
  ...
  perform set_config('request.jwt.claims', v_mclaims, true);
  set local role authenticated;
  begin perform public.calculate_batch_row(v_row, v_cvn, v_res,
      'qca/1~t1~' || app_private.qca_ts(v_now) || '~' || app_private.qca_ts(v_now + interval '60 seconds')
      || '~' || repeat('0',64)); v_state:='NO ERROR';
  exception when others then v_state := sqlerrm; end;
  reset role;
  return next is(v_state, 'attestation_invalid',
  ...
  return next ok(v_before = v_after, 'CP-101e and the COMPLETE stored calculation is unchanged after all five attempts');

  -- ═══════════════════ F. CP-109 exact-text JSON ══
  perform set_config('request.jwt.claims', v_mclaims, true);
  set local role authenticated;
...
begin
  ...
  delete from public.plant_construction_adoptions where construction_version_id in
    (select id from public.construction_versions where construction_id in
      (select id from public.constructions where name = '__p2 s7r con'));
  delete from public.construction_versions where construction_id in
    (select id from public.constructions where name = '__p2 s7r con');
  delete from public.constructions   where name = '__p2 s7r con';
...
create or replace function tests.calculation_writer()
returns setof text language plpgsql set search_path = 'extensions', 'pg_catalog' as $fn$
begin
  return query select * from tests.__s7r_body();
  perform tests.__s7r_teardown();
  return next is((select count(*)::int from app_private.attestation_keys), 0,
    'CP-115 the persistent keyring is EMPTY after the suite - the test key existed only inside this transaction, and no migration has ever inserted one');
end $fn$;

revoke all on function tests.__s7r_sign(text,bytea,text,bigint,bigint,bigint,integer,bigint,text,text,text,text,timestamptz,timestamptz)
...
revoke all on function tests.calculation_writer()  from public, anon, authenticated;