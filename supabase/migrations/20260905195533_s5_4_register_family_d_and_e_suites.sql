-- S5-4: register the three S5 suites in the regression entry point and remove
-- the scratch probes used to run them standalone during development. After this
-- tests.run_all() is again the single command that states the whole position.
...
drop function if exists tests.__probe_md();
drop function if exists tests.__probe_mr();
drop function if exists tests.__probe_pb();