-- S9-P/11b: tests.batch_workspace() supplies the Pricing Date its inserts now need.
--
-- Same cause and same remedy as S9-P/11a. S9-P/10 removed the server-date
-- default from batches.pricing_date; this suite inserts into batches directly at
-- OWNER level and did not name the column. It now computes the PRODUCING PLANT's
-- local date, exactly as create_batch does.
--
-- NOT ONE ASSERTION CHANGES. Only one new variable, one lookup and the two
-- fixture inserts. Every BF-1..BF-19 verdict is byte-identical - including BF-5,
-- the foreign-key index-coverage gate that caught the composite-index defect in
-- S9-P/3 and must keep running unchanged.
...
begin
  ...
  -- S9-P/11b: the plant-local Pricing Date, the same rule create_batch follows
  ...
  select v_owner, c.id, v_owner from public.capabilities c
   where c.capability_key = 'manage_customer_master'
     and not exists (select 1 from public.group_capability_grants g
                      where g.app_user_id = v_owner and g.capability_id = c.id);
  ...
  insert into public.batches (batch_reference, family_id, plant_id, owner_user_id, created_by, pricing_date)
    values ('client-supplied', v_fam, v_nag, v_owner, v_owner, v_pdate) returning id, batch_reference into v_batch, v_ref;
  insert into public.batches (batch_reference, family_id, plant_id, owner_user_id, created_by, pricing_date)
  ...
  select content_version into v_party_cv from public.parties where id = v_party;
  perform app_private.reassign_party_family(v_party, v_fam, v_party_cv, current_date);
  ...
  -- deliberately NOT reverted: a permanent code is never released and
  -- published is terminal (CDM-03/CDM-12). Nothing below needs it proposed
  -- again, and the cleanup removes it outright.
