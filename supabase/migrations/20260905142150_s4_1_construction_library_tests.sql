-- S4-1: executable proof gates for the Construction Library.
--
-- PC-11 is the gate CDM-13 exists for. It is a PATTERN scan, not four literal
-- column names, so a later migration adding `wastage_pct` or `conversion_rate`
-- fails it too. That is the difference between asserting the decision and
-- asserting today's spelling of it.
...
begin
  ...
  foreach t in array v_tables loop
    return next ok(
      ...
      format('PC-1 %s has RLS enabled AND forced', t));
    return next ok(
  ...
  return next is(
    ...
    1, 'PC-4 exactly one permissive policy per Construction table and action');
  ...
  return next is(
    ...
    0, 'PC-24 every single-column foreign key in the Construction Library is indexed');
  ...
  return next is(
    ...
    0, 'PC-11 NO waste or conversion column of any spelling exists on construction_versions');

  return next is(
    (select count(*)::int from information_schema.columns
      where table_schema='public' and table_name='constructions'
        and (column_name ~* 'wast' or column_name ~* 'conv')),
    0, 'PC-11a and none on constructions either - the hidden tier has no destination');
  ...
  begin
    ...
    return next fail('PC-5 a published Construction without a code should be rejected');
  exception when others then
    return next ok(true, 'PC-5 published Construction without a permanent code rejected ('||sqlstate||')');
  end;

  begin
    insert into public.constructions (name, status, created_by)
    values ('__p2 pc probe','merged',v_owner);
    return next fail('PC-6 a merged Construction without a survivor should be rejected');
  exception when others then
    return next ok(true, 'PC-6 merged Construction without retained lineage rejected ('||sqlstate||')');
  end;

  begin
    insert into public.constructions (name, construction_code, status, created_by)
    values ('__p2 pc probe','CON-12','published',v_owner);
    return next fail('PC-7 a malformed Construction Code should be rejected');
  exception when others then
    return next ok(true, 'PC-7 malformed Construction Code rejected ('||sqlstate||')');
  end;
  ...
  return next is((select count(*)::int from public.constructions where name = '__p2 pc dup'), 2,
                 'PC-8 Construction names are deliberately NOT unique (CDM-12/DM-143)');
  ...
  return next is((select board_gsm from public.construction_versions where id = v_v), 430.00,
                 'PC-12 an UNapproved version is editable');
  ...
  begin
    ...
    return next fail('PC-13 an approved Construction Version must not be editable');
  exception when others then
    return next ok(true, 'PC-13 approved Construction Version is immutable ('||sqlstate||')');
  end;

  -- The point of the trigger: everything above ran as the table owner, not as
  -- `authenticated`, so PC-13 proves immutability holds exactly where the RLS
  ...
  return next ok(
    ...
    'PC-14 immutability is a trigger, so it binds BYPASSRLS roles too, not only authenticated');

  begin
    update public.construction_versions set version_no = 2 where id = v_v;
    return next fail('PC-15 version_no must be immutable');
  exception when others then
    return next ok(true, 'PC-15 a version cannot be renumbered ('||sqlstate||')');
  end;
  ...
  return next is((select count(*)::int from pg_catalog.pg_constraint
                 ...
                 'PC-16 approval and its attribution cannot be separated (CDM-34)');
  ...
  begin
    ...
    return next fail('PC-9 a permanent Construction Code must never change');
  exception when others then
    return next ok(true, 'PC-9 an allocated Construction Code is permanent ('||sqlstate||')');
  end;
  begin
    update public.constructions set construction_code = null where id = v_k;
    return next fail('PC-9a a permanent code must never be released back to null');
  exception when others then
    return next ok(true, 'PC-9a a code is never released back to the pool ('||sqlstate||')');
  end;
  begin
    update public.constructions set status = 'proposed' where id = v_k;
    return next fail('PC-10 published -> proposed must be rejected');
  exception when others then
    return next ok(true, 'PC-10 the Construction lifecycle is exhaustive and published is terminal ('||sqlstate||')');
  end;
  ...
  return next is(v_seen, 0,
    'PC-17 a Maker WITHOUT read_construction_library sees no Constructions at all');
  ...
  reset role;
  return next ok(v_ok, 'PC-18 but a make_quote holder MAY propose a Construction (CDM-12/DM-144)');
  ...
  reset role;
  return next ok(not v_ok, 'PC-19 and may NOT insert a published Construction - the branch is narrow');
  ...
  reset role;
  return next ok(not v_ok, 'PC-20 a Maker may not edit the shared Construction Library');
  ...
  return next ok(not v_ok,
    'PC-21 a Maker without adopt_construction_for_plant cannot adopt, even at their own plant');
  ...
  reset role;
  return next ok(v_ok, 'PC-22 the explicit plant grant is what admits the adoption');
  ...
  reset role;
  return next ok(not v_ok,
    'PC-23 and confers nothing at PUN - cross-plant isolation holds on the row own plant_id');
...
revoke all on function tests.construction_library() from public;
revoke all on function tests.construction_library() from anon;
revoke all on function tests.construction_library() from authenticated;
