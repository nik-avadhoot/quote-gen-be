-- S4-2: executable proof gates for the SKU master.
--
-- PS-15 is the gate that carries the Construction-authority rule as far as S4
-- honestly can. CDM-13's mutual exclusivity is enforced on `batch_rows`, which
-- is Family F and does not exist until S6. What S4 CAN do is prove the two sides
-- are structurally distinguishable and that the exact discriminator S6's trigger
-- will use returns the right answer against real rows. It does that here rather
-- than deferring the whole rule to a later slice with nothing shown.
...
begin
  ...
  update public.app_users set status = 'inactive' where id = v_uid;
  perform pg_catalog.set_config('request.jwt.claims', v_claims, true);
  set local role authenticated;
...
revoke all on function tests.sku_master() from public;
revoke all on function tests.sku_master() from anon;
revoke all on function tests.sku_master() from authenticated;