-- S9-P/12: prove the direct write paths are gone (D-T, D-U).
--
-- TWO CHANGES TO THE SUITE BODY, AND NOTHING ELSE.
--
-- 1. CP-31a is REWRITTEN (D-U). It previously asserted that a DIRECT UPDATE
--    naming a foreign-plant Release was refused by fk_batch_pricing_basis BY
--    NAME. After S9-P/9 that write is refused earlier, by privilege, so the old
--    assertion failed - correctly, but for a new reason. The authenticated path
--    now proves the intended 42501, and the composite foreign key is proved
--    SEPARATELY at owner level in CP-31b, inside a rolled-back subtransaction,
--    so database integrity stays independently verified rather than merely
--    inferred from a privilege refusal.
--
-- 2. A new section E proves the revocation itself: every S9-P pricing field, the
--    status transition S9(b) owns, direct INSERT, and the privilege inventory.
--    Each mutation gate captures the WHOLE row as jsonb before and after,
--    because "the statement failed" and "the row is unchanged" are different
--    claims and only the second one is the guarantee.
--
-- WHY THE MIXED-COLUMN GATE READS AS IT DOES. The brief asks for a mixed
-- protected/unprotected UPDATE. After full revocation there is no unprotected
-- column left on batches - that is the point of D-T - so CP-93 names a
-- pre-existing S6 column alongside a governed pricing column and proves the
-- statement fails as a unit with neither half applied. Stating that plainly is
-- better than inventing an "unprotected" column that no longer exists.
...
begin
  ...
  return next is(
    ...
    0,
    'CP-96b and NO column-level INSERT or UPDATE either - the bypass surface is closed at both levels');
  return next is(
